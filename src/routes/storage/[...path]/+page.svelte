...
<div class="flex h-screen">
    <div class="md:flex flex-col px-3 justify-between hidden bg-gray-300 static w-60">
        <div>
            <div class="mb-5">
    ...
    <div class="w-full">
        <div class="px-10 pb-5 md:pl-0 bg-gray-300">
        ...
        <div class="flex gap-5 md:hidden my-5 mx-10 justify-between">
            <div class="w-full">
                <button type="button" on:click={()=> createDirDialog.showModal()} class="button bg-black text-white text-lg rounded-md py-1 px-2 text-center w-full">Создать папку</button>
            </div>
            <form method="POST" action="?/upload" enctype="multipart/form-data" class="w-full">
                <label for="file">
                    <p class="button bg-black text-white text-lg rounded-md py-1 px-2 text-center w-full cursor-pointer">
                        Загрузить файл
                    </p>
                    <input type="file" name="file" id="file" class="hidden" onchange="form.submit()" /> 
                </label>
